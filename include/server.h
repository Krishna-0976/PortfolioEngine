...
private:
    ...
    atomic<long long> totalPriceRequests{0};
    atomic<long long> cacheServedRequests{0};

public:
    PortfolioServer(AlertEngine& eng, PortfolioMonitor& mon, Database& database, const string& key)
    ...
    void start() {
        ...
        svr.Get("/price", [this](const httplib::Request& req, httplib::Response& res) {
            string ticker = req.has_param("ticker") ? req.get_param_value("ticker") : "";
            double price = -1;
            if (!ticker.empty()) {
                totalPriceRequests++;
                ...
                if (price > 0) {
                    ...
                    string path = "/api/v1/quote?symbol=" + ticker + "&token=" + apiKey;
        ...
        svr.Get("/summary", [this](const httplib::Request&, httplib::Response& res) {
            ...
            for (auto& s : stocks) {
                ...
                totalPnL   += pnl;
                if (pricedCount == 0) firstPnL = pnl;
                else if (pnl != firstPnL) allTied = false;
                pricedCount++;
                if (pnl > bestPnL)  { bestPnL  = pnl; best  = s.ticker; }
                if (pnl < worstPnL) { worstPnL = pnl; worst = s.ticker; }
            }
            if (pricedCount < 2 || allTied) { best = "N/A"; worst = "N/A"; }
            json result = {
        ...
        svr.Post("/alert", [this](const httplib::Request& req, httplib::Response& res) {
            auto body = json::parse(req.body);
            string ticker    = body["ticker"];
            string condition = body["condition"];
        ...
        svr.Get("/history", [this](const httplib::Request& req, httplib::Response& res) {
            int limit = 200;
            if (req.has_param("limit")) try { limit = stoi(req.get_param_value("limit")); } catch (...) {}
            json result = json::array();
